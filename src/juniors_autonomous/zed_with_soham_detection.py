...
class auto:
    def __init__(self):
        ...
        self.objp = np.zeros((4*4,3), np.float32)
        self.objp[:,:2] = np.mgrid[0:4,0:4].T.reshape(-1,2)
        
        self.objpoints = []  
        self.imgpoints = []
        
        self.mtx = None  # Camera matrix and distortion coefficients
        self.dist = None
        
        # Template for arrow detection
        self.template = cv2.imread('arrow_template.jpg', cv2.IMREAD_GRAYSCALE)
        assert self.template is not None, "Template file could not be read"
    ...
    def calibrate_camera(self, images_path='*.jpg'):              
    ...
    def arrowdetectlessthan3(self, image, depth):
        if self.mtx!=None:
            ...
            max_val_c, min_val_c, min_loc_c, max_loc_c = 0.0, 0.0, 0.0, 0.0
            i_max = 0.0
            vals = np.arange(0.05, 1, 0.03)
            
            for i in vals:
                try:
                    template2 = cv2.resize(
                        self.template, None, fx=i, fy=i, 
                    ...
                    res = cv2.matchTemplate(img2, template2, method)
                    
                    min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(res)
                    if float(max_val) > float(max_val_c):
                        min_val_c, max_val_c, min_loc_c, max_loc_c = (
            ...
            if max_loc_c != 0 and float(max_val_c) >= 0.61:
                top_left = max_loc_c
                bottom_right = (top_left[0] + w_final, top_left[1] + h_final)
                
                # Draw rectangle
                cv2.rectangle(img2, top_left, bottom_right, 120, 2)
                
                # Calculate point and depth
                point = (
                ...
                # Depth calculation using known height
                img_height = abs(top_left[1] - bottom_right[1])
                known_height = 21
                f = self.mtx[0,0]
                calculated_depth = (f * known_height) / img_height
