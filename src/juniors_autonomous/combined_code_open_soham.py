...
#=========================This code is for differential rotation checking the frequency of opencv using a counter using cropped template======================
...
class ZedDepth:
    def __init__(self):
        ...
        self.templatel = cv.GaussianBlur(cv.imread('arrow_template_left_crop1.jpg', cv.IMREAD_GRAYSCALE),(5,5),3)
        self.templater = cv.GaussianBlur(cv.imread('arrow_template_right_crop1.jpg',cv.IMREAD_GRAYSCALE),(5,5),3)
    ...
    def move_straight(self):
        ...
        if self.init or self.searchcalled:
            ...
            if abs(self.distance)> self.circle_dist:
                ...
                if(self.arrow_numbers == self.count_arrow):
                    self.v1_competition()
                ...
                if self.direction == "Not Available":
                    ...
                    if self.count_arrow <= self.arrow_numbers:
                        ...
                        file_object.close()
                        with open("sparsh1.csv", "a", newline="") as csv_file:
...
#        if abs(self.enc_data) < 0.6 * self.angle_thresh and not self.init:
#            print("I hit empty return")
            ...
            if (
                ...
                if self.distance <1.5 and not self.searchcalled:
                    if self.cvcounter<10:
                        ...
                        print(f"I got {direction} at my {self.cvcounter} run")
                        print(f"%%%%%%LEFT ARROW DETECTED :{self.arrow_left} and RIGHT ARROW DETECTED :{self.arrow_right} and we have ret == {ret}&&&&&&&&&&&&&&&&&&&&&&")
                        if ret == True:
                            self.temp_ret = ret
                        ...
                        print(f"IN ELSE CVCOUNTER ================ {self.cvcounter}")
                        if self.temp_ret:
