# -*- coding: utf-8 -*-
"""zed_became_imu.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1tP87ZaVnkTIwzqn1aiHhpaTrM_ydZcmH
"""
...
# search_called -- checks for process dict called or not  , not to check if search is called
#ret - is used to detect if arrow is detected or not  -- is set true within arrow detect less than or more than 3
# init is used for for checking if search is initialised or not if camera turned 60 in while loop set to true  ( search happens only is init is true )
# we should stop using more_than_3 as soon as distance comes out to be less than 3


# why we change 60 to 47  ---- why is there a convuluted thing of doing 55 - 2*4
class auto:
    def __init__(self):
        ...
        self.angle_thresh = 4
        self.kp = 20    #constants used during P control
        self.kp_rot = 1.5
        ...
        self.arrow_seen = False     #Boolean flag to check whether arrow was successfully
        self.initial_yaw = 0.0
        self.rotate_angle = 90

        self.angles_dict = defaultdict(list)
        self.pro_dict_called = False

        self.arrow_numbers = 4    #what is arrow numbers doing exactly?
        ...
        self.time_thresh = 5
        self.stm_pub = rospy.Publisher("stm_write", std_msgs.Int32MultiArray, queue_size=10)
        self.init = False   #used for for checking if search is initialised or not if camera turned 60 in while loop set to true  ( search happens only is init is true )
        self.start_angle = 55
        self.angle_thresh = 4
        self.count_arrow = 0
        ...
        self.dd=True
        self.dd_pub = rospy.Publisher("turningStyle", bool, queue_size=10)
        self.rot_pub = rospy.Publisher("joy", Joy, queue_size=10)
    ...
    def arrowdetectlessthan3(self, image, depth):   #Using OpenCV
        ...
        if maxVal > 0.5:
            ...
            try:      #why the try except?
                ...
                print(point)
                self.arrow_seen = False  #no arrow detected
                # self.search()
                distance = 2.5
    ...
    def cone(self, image):  #Cone detection
    ...
    def arrowdetectmorethan3(self):   #Using YOLO
    ...
    # self.arrow_seen,self.direction,pix,self.distance
    def search(self):
        print("=================================================================SEARCH START====================================================================")
        print("self.pro_dict_called (should print false here always):", self.pro_dict_called)
        if abs(self.enc_data) < 0.6 * self.angle_thresh and self.arrow_seen and not self.init:
            # if arrow is detected and realsense is facing straight, then come out of search immediately.
            return

        #self.pro_dict_called = True
        ...
        wheelrpm_msg = WheelRpm()
        wheelrpm_msg.hb = False
        wheelrpm_msg.omega = 0
        wheelrpm_msg.vel = 0
        wheelrpm_pub.publish(wheelrpm_msg)
        print("Rover has stopped.")

        stm_msg = std_msgs.Int32MultiArray()
        stm_msg.data = [0, 0, 0, 0, 0, 0]
        stm_msg.layout = std_msgs.MultiArrayLayout()
        stm_msg.layout.data_offset = 0
        stm_msg.layout.dim = [std_msgs.MultiArrayDimension()]
        stm_msg.layout.dim[0].size = msg.layout.dim[0].stride = len(msg.data)
        stm_msg.layout.dim[0].label = "write"
        self.stm_pub.publish(stm_msg)
        ...
        print("self.arrow_seen:", self.arrow_seen)
        if (
            self.init == True
            and self.enc_data > -60
            and not self.arrow_seen
        ):
            ...
            self.distance = float(round(self.distance))     #calculating distance of arrow from rover
            ...
            print()
        elif not self.arrow_seen or abs(self.enc_data) > self.angle_thresh:
            ...
            stm_msg.data = [0, 0, 0, 0, 0, 0]
            self.stm_pub.publish(stm_msg)
            self.init = False   #stop searching
            self.pro_dict_called = True    #to enter process dict
            self.distance = 10.0
            self.start_time = time.time()

        print("======================================================SEARCH END=========================================================")

    def process_dict(self):     #Do indentation later
        print("self.pro_dict_called (should print true always):", self.pro_dict_called)
        # if not self.init and self.pro_dict_called:
        # the first part is not needed, because whenever self.pro_dict_called is set to true, self.init is set to false
        # actually, this if is only not needed, because in main(), there is an if self.pro_dict_called(), then process_dict(), which takes care of everything
        ...
        try:
            ...
            if min(self.angles_dict.keys()) != 0:
                print("Entered if")
                # del self.angles_dict['0.0']
                max_length_key = min(self.angles_dict.keys())   #why is max written?
        ...
        if len(self.angles_dict[max_length_key + 1]) != 0:   #does readings from 2 distances make it more accurate?
    ...
    def convex_hull_pointing_up(self, ch):  #to detect the cone
    ...
    def cone_model(self):     #YOLO Detection for cone this time
    ...
    def main(self):
        ...
        if (
            ...
            if (not self.turn and not self.arrow_seen) or (self.init):
                self.search()
                if self.pro_dict_called and not self.init:
                    self.process_dict()

            if not self.turn:
                self.arrow_seen, direction, pix, self.distance = self.cone_model()
                if self.arrow_seen == True:
                    print("Cone detected")
            ...
            if (
                ...
                ret, direction, pix, self.distance = self.arrowdetectmorethan3()
                #if self.arrow_seen == True:
                ret, depth_frame, color_frame, depth = self.get_frame()
                ret, direction, pix, distance = self.arrowdetectlessthan3(
                        color_frame, depth
                    )
                    # if not self.arrow_seen:
                    #     self.arrow_seen,self.direction,pix,self.distance=self.arrowdetectmorethan3()
                if ret == True:
                    self.arrow_seen = ret
...
# Pitch (y-axis rotation)
    ...
    def move_straight(self):
        ...
        if self.init or self.pro_dict_called:
            print("move_straight() is being ignored due to search().")
            msg.vel = 0
            wheelrpm_pub.publish(msg)
        elif self.arrow_seen:
    ...
    def rotate(self, dir):
        msg = WheelRpm()
        if(self.dd==True):
            self.dd_pub.publish(True)
            print("Differential Rotation has been called")
            msg.vel = 0
            msg.hb = False
            ...
            self.dd_pub.publish(False)
            print("Steering rotation has been called")
            diff = self.z_angle - self.initial_yaw
            if diff > 180:
                diff = diff - 360
            elif diff < -180:
            ...
            if abs(error) <= 0.5 * self.angle_thresh:
                ...
                joy_msg = Joy()
                joy_msg.axes = [0.0] * 8  # Adjust size to match the Joy message used in the original script
                joy_msg.buttons = [0] * 15
                joy_msg.buttons[self.rotinplace_btn] = 1

                self.rot_pub.publish(joy_msg)
                rospy.loginfo("Published simulated high message")
