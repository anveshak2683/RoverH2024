...
model1 = YOLO("/home/nvidia/caesar2020/src/juniors_autonomous/best.pt")
model2 = YOLO("/home/nvidia/caesar2020/src/juniors_autonomous/best1.pt") 
...
class ZedDepth:
    def __init__(self):
        ...
        try:
            ...
            print("3")
            rospy.Subscriber("/gps", std_msgs.Float32MultiArray, self.gps_callback)
            print("4")
    ...
    def color_callback(self, msg):
        ...
        try:
            ...
            #alpha = put contrast value b/w 1.0 and 3.0
            #beta = put brightness value b/w 0 and 100
            #self.color_image = cv.convertScaleAbs(self.color_image,alpha=alpha,beta=beta)
    ...
    def get_box(self):
        self.results1 = model1.predict(self.color_image, conf=0.5, max_det=2)
        self.results2 = model2.predict(self.color_image, conf=0.5, max_det=2)
        ...
        if self.results1!=None:        	
            for r in self.results1:
                    ...
                    for box in boxes:
                        ...
                        self.latest_xmin1 = left
                        self.latest_xmax1 = right
                        self.latest_ymin1 = top
                        self.latest_ymax1 = bottom
       ...
       if self.results2!=None:        	
            for r in self.results2:
                    ...
                    for box in boxes:
                        b = box.xyxy[0]
                        left, top, right, bottom = map(int, b)
                        self.latest_xmin2 = left
                        self.latest_xmax2 = right
                        self.latest_ymin2 = top
                        self.latest_ymax2 = bottom
                    cv.imshow('text', self.annotated_image)
                    print(f"Exited Forloop. self.ret = {self.ret}")
            cv.waitKey(1)
        if self.results1 is None and self.results2 is None:
            self.ret = False
	else:
            self.ret = True
            ...
            if abs(self.distance)> self.circle_dist:
                ...
                if self.direction == "Not Available":
                    ...
                    if self.count_arrow <= self.arrow_numbers:
                        ...
                        self.latlong[0].append(lat_av)
                        self.latlong[1].append(lon_av)

                        file_object = open("coordinates.txt", "w")
                        file_object.write(
                        "latitude :%f, longitude :%f", lat_av, lon_av
                            )   
                        file_object.close()
                        with open("sparsh.csv", "a", newline="") as csv_file:
    ...
    def search(self):
        ...
        if (
            ...
            print()
        elif (not self.ret or (abs(self.enc_data) > self.start_angle)) :
...
# Pitch (y-axis rotation)
    ...
    def gps_callback(self, msg):
        if msg.data[0] and msg.data[1]:
            self.current_latitude = msg.data[0]
            self.current_longitude = msg.data[1]
            '''
            file_object = open("coordinates.txt", "w")
            file_object.write(
                "latitude :%f, longitude :%f", self.current_latitude, self.current_longitude
            )
            file_object.close()
            with open("coordinates.csv", "a", newline="") as csv_file:
                csv_writer = csv.writer(csv_file)
                csv_writer.writerow([self.current_latitude, self.current_longitude])
            '''
    def main(self):
        gps_data_pub.publish(self.gpscalled)
        ...
        if (
            ...
            if (
                ...
                if self.distance < 2.0 and not self.searchcalled:
