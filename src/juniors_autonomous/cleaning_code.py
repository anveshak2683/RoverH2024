...
#from tf2 import euler_from_quarternion
...
class auto:
    def __init__(self):
        ...
        self.ret = False		#Boolean flag to check whether arrow was successfully
    ...
    def get_frame(self):      #Getting frames from realsense
    ...
    def arrowdetectlessthan3(self, image, depth):   #Using OpenCV
        # _,image=self.cap.read()
        print("*****************LESS THAN 3 METERS CODE ******************************************************************")
        ...
        for scale in np.linspace(0.06, 1.0, 70)[::-1]:
            ...
            r = gray.shape[1] / float(resized.shape[1])   #scale
            if resized.shape[0] < self.h or resized.shape[1] < self.w:
                break
            # cv2.imshow("",resized)
            result = cv2.matchTemplate(resized, self.template_r, cv2.TM_CCOEFF_NORMED)    #Template Matching
        ...
        if maxVal > 0.5:
            ...
            try:      #why the try except?
                distance = depth.get_distance(point[0], point[1])
            except:
                print(point)
                self.ret = False  #no arrow detected
                # self.search()
                distance = 2.5
    ...
    def cone(self, image):
        ...
        for c in contours:
            ...
            bounding_rect = []
            for ch in hull:
                if self.convex_hull_pointing_up(ch):    #method used below (to check if it is actually a cone)
    ...
    def search(self):
        ...
        if time.time() - self.start_time < self.time_thresh:  # time_thresh is 5s
            print(
                "time.time()-self.start_time (when this becomes 5s, search will happen):",
        ...
        if (
            ...
            if self.distance > 0.0 and self.distance < 15.0:  # change
            ...
            print()
        elif not self.ret or abs(self.enc_data) > self.angle_thresh:
...
# Pitch (y-axis rotation)
    ...
    def move_straight(self):
        ...
        if (
            abs(self.initial_drift_angle - self.z_angle) > 5 and False
