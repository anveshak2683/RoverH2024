...
class IK_IROC():
    def __init__(self):
        ...
        self.input = int(input("should I catch or drop? 1 for catch -1 for drop"))
    ...
    def angle_value(self):
       ...
       # print(np.arccos(self.pz/(2*k)))
        ...
        if (abs(self.enc_data[0] - base) <= 3 and abs(self.enc_data[1] - shoulder) <= 3 and abs(self.enc_data[2] - elbow) <= 5) or time.time()- self.start_time > 120:
            print("Goal Reached")
            self.goal_reached = True
            self.goal_counter = self.goal_counter + 1
 #           if self.goal_counter ==1:
#            	self.listen_to_tanish =False
            if self.goal_reached >= 2:
