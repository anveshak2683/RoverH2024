...
class Node:
    ...
    def joyCallback (self, msg):
       ...
       # outbuff += [ (msg.buttons[i] - msg.buttons[i+2]) * 0xFF for i in range(4, 6) ]
        ...
        axes = [ int (msg.axes[i] * 0xEF) for i in range(4) ]
        buttons = [ int((msg.buttons[12]-msg.buttons[13])*255)]
        buttons.append(int((msg.buttons[14]-msg.buttons[15])*255))
       ...
       #HASA
        ...
        if msg.buttons[0] == 1:
            self.prev_aut_mode = 1
        else:
            if (self.prev_aut_mode == 1):
