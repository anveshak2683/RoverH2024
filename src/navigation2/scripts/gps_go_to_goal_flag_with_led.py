...
from std_msgs.msg import Int32, Int8,Int32MultiArray, MultiArrayLayout, MultiArrayDimension
import time
...
class GoToGoal():
    def __init__(self):
        ...
        self.repeater_pub = rospy.Publisher('/stm_write', Int32MultiArray, queue_size=10)
...
#        print("self.current_yaw", self.current_yaw)
    ...
    def move_to_goal(self):
        ...
        if abs(distance_travelled -self.distance) < 0.5:
            ...
            while time.time()-self.start_time < 3:
                print("waiting to drop repeater")
                rospy.sleep(0.5)
            msg = Int32MultiArray()
            ...
            #msg.data = [0,0,0,-255,0,0]  #assuming the repeater motor is connected to the outbuff 1
            self.start_time  = time.time()
            while time.time() - self.start_time < 3 and self.repeater_drop == False:
                print("dropping repeater now")
                msg.data=[0,0,0,-255,0,0]
                self.repeater_pub.publish(msg)
            self.repeater_drop = True
            print("finished dropping repeater")
            #for i in range(10):
            msg.data = [0,0,0,0,0,0]
            self.repeater_pub.publish(msg)
            ...
            if (self.current_yaw - self.angle_to_turn) < -4:
                ...
                vel_msg.vel= 0
                # self.velocity_publisher.publish(vel_msg)
            elif abs(self.current_yaw - self.angle_to_turn) < 4 and self.second_gps_received == False:
                #vel_msg = WheelRpm()
                vel_msg.vel = 30
                vel_msg.omega= 0
                ...
                vel_msg.omega = 0
                # self.velocity_publisher.publish(vel_msg)
            if distance_travelled <=10 and distance_travelled >=7:
                vel_msg.omega = 0
        self.velocity_publisher.publish(vel_msg)
        self.rate.sleep()
    ...
    def main(self):
        if (self.rscp_received == True) or self.second_gps_received == True:
            ...
            if self.starting_move==False:
                start_timer=time.time()
                while time.time()-start_timer<20:
                    msg=WheelRpm()
                    msg.vel=30
                    msg.omega=0
