...
#                i1=cv2.rectangle(self.cframe,(self.x,self.y),(self.x+self.w,self.y+self.h),(255,0,0),2,0)
                ...
                d2 = 0
                d2 = self.get_depth(self.x+self.w/2, self.y+self.h/2)
                print("d2", d2)
                p_x = self.x + self.w/2
                g = WheelRpm()
                kp = 26.67
                if p_x > 370:
                    g.omega = -15
                elif p_x <270 and p_x>0:
                    g.omega = 15
                elif p_x >270 and p_x<370:
                    g.omega = 0    
                    self.goal_reached_angular = True
                if d2 > 0.75 and self.goal_reached_angular == True:
                    print("I came inside vel publishing")
                    g.vel =20
                elif d2 > 0.5 and d2 < 0.75 and self.goal_reached_angular == True:
                    g.vel = int(min(kp * d2, 20))
                #g.vel = 0
                elif d2 < 0.4 and self.goal_reached_angular == True:
    ...
    #                    if msg.data[0]!=0 and msg.data[1]!=0 and self.ik_start:
                ...
                if len(self.check) >=3:
                    ...
                    msg.data = [-self.cord_y - 0.13, self.cord_z-0.05, self.cord_x+0.15]
                    goal_pub.publish(msg)
                else:
                    pass               	
    def get_depth(self,x,y):
        if x<360 and y<640:
            ...
            dframe = np.asanyarray(self.dframe)
            depth=dframe[int(y),int(x)]
            return depth
    def get_coords(self):
