...
from navigation2.msg import auto , detection
import sys
import crc
...
class FrameParser:
    ...
    def process(self, read_bytes: bytes):
        ...
        try:
            ...
            print("checksum error send message again",e)
            self.buffer = bytearray()
...
class RSCP_Receiver:
    ...
    def detection_callback(self,msg : detection):
        ...
        self.frame=self.dedect_frame(msg.depth,msg.color)
        self.ser.write(self.frame)
    ...
    def on_update(self,frame):
        ...
        # frame.msg_id=b"\x01"
        # frame.length=b"\x01"
        # print(bytes(frame.start_byte)+frame.msg_id.to_bytes(1,"big")+frame.length.to_bytes(1,"big")+bytes(frame.data))
        
        # print(crc.Calculator(crc.Crc16.XMODEM.value).checksum(bytes(frame.start_byte)+frame.msg_id.to_bytes(1,"big")+frame.length.to_bytes(1,"big")+bytes(frame.data)))
        # if frame.checksum == crc.Calculator(crc.Crc16.XMODEM.value).checksum(bytes(frame.start_byte)+frame.msg_id.to_bytes(1,"big")+frame.length.to_bytes(1,"big")+bytes(frame.data)):
    ...
    def setparameters_body(self):
        ...
        self.rscp_data.latitude = self.body.parameters["airlock_coordinates"]["lat"]
        self.rscp_data.longitude = self.body.parameters["airlock_coordinates"]["lon"]
        self.rscp_data.msg_id = self.msg_id
        self.rscp_data_pub.publish(self.rscp_data)
        print(f"Body of the Message = {self.body}")     
