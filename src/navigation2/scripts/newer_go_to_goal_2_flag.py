...
class GoToGoal():
    def __init__(self):
        ...
        self.current_latitude, self.current_longitude = 0.0,0.0
    ...
    def gps_callback(self, msg):
        if self.current_latitude ==0.0 and self.current_longitude == 0.0:
    ...
    def rscp_callback(self, data):
        if data.stage_id == 3:
            if data.msg_id == 2:
                self.goal_latitude = data.latitude
                self.goal_longitude = data.longitude
    ...
    def align_to_goal(self):
        a = math.atan2(self.goal_longitude - self.initial_longitude, self.goal_latitude - self.initial_latitude)
...
#        msg = WheelRpm()
        ...
        if self.angle_to_turn <= 0:
            ...
            self.dummy = 1
        msg = WheelRpm()
        if self.dummy != 0 and abs(self.angle_to_turn - self.yaw_angle) >4:
            msg.omega = 20*self.dummy
            msg.vel = 0
    ...
    def spin(self):
        while not rospy.is_shutdown():
            ...
            if self.aligned_to_goal==False and self.code_stopping_variable == False and self.execute_goal==True:
