...
class RSCP_Receiver:
    def __init__(self) -> None:
        rospy.init_node("pozhilan",anonymous = True)
        self.data=None

        self.rscp_data=auto()

        self.task_completed_sub= rospy.Subscriber("/check", Int32, callback= self.task_completed_callback)
    ...
    def task_completed_callback(self,check) :
        ...
        if check.data==1 and self.check_received == False:
            self.check_received = True
            self.frame=self.task_completed_frame()
            self.ser.write(self.frame)

    def detection_callback_colour(self,check):
        self.frame=self.dedect_frame()
        self.ser.write(self.frame)

    def detection_callback_distance(self,check):
        self.frame=self.dedect_frame()
        self.ser.write(self.frame)
