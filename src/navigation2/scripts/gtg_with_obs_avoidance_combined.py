...
class GoToGoal():
    def __init__(self):
        self.goals_x = [8.0,8.0]
        self.goals_y = [0.1,3.0]
        ...
        self.left_turn_happening = False
        self.right_turn_happening = False
        self.initial_yaw = 0
   ...
   #     self.ik_start_tanish_prev=self.ik_start_tanish
    ...
    def move_to_goal(self):
        ...
        if self.count < 20:
            ...
            self.left_turn_happening = False
            self.right_turn_happening = False
        elif self.right_turn_happening == True:
            vel_msg = self.move_to_goal_right()
        elif self.left_turn_happening == True:
            vel_msg = self.move_to_goal_left()
        else:
            a_left = self.tensor[self.left_45:self.left_90]
            a_right = self.tensor[self.right_90:self.right_45]
            a_left_count = a_left < 5
            a_left_prime = torch.nan_to_num(a_left_count, posinf=500,neginf=500)
            a_left_with_dist = torch.nonzero(a_left_prime).to(device)
            num_left_points = a_left_with_dist.size(dim=0)

            a_right_count = a_right < 5
            a_right_prime = torch.nan_to_num(a_right_count, posinf=500,neginf=500)
            a_right_with_dist = torch.nonzero(a_right_prime).to(device)
            num_right_points = a_right_with_dist.size(dim=0)
            
            print(f"Number of points on the left = {num_left_points} and number of points on the right = {num_right_points}")
            if(num_left_points <= num_right_points):
                vel_msg = self.move_to_goal_left()
                self.left_turn_happening = True
            else:
                vel_msg = self.move_to_goal_right()
                self.right_turn_happening = True
    ...
    def move_to_goal_left(self):
        vel_msg = WheelRpm()    
        if self.first_stop == False and self.third_stop == False:
    ...
    def move_to_goal_right(self):
        vel_msg = WheelRpm()
        if self.first_stop == False and self.third_stop == False:
            if(self.turn_over == False): #turning over
                print(self.left_90, self.middle_0)
                a_straight = self.tensor[self.right_90:self.left_45]
                a = a_straight<2.0
                a_prime = torch.nan_to_num(a, posinf = 500, neginf = 500).to(device)
                a_with_dist = torch.nonzero(a_prime).to(device)
                ...
                a_straight = self.tensor[self.middle_0:self.left_90]
                #print(a_straight)
                a = a_straight<1.2
                a_prime = torch.nan_to_num(a, posinf = 500, neginf = 500).to(device)
                a_with_dist = torch.nonzero(a_prime).to(device)
            ...
            self.check_if_object_in_front()
            a_prime = torch.nan_to_num(self.tensor[self.middle_0:self.left_90], posinf = 500, neginf = 500).to(device)
            #print(a_prime)
            dist_to_chk = 2.0
            if(self.y_disp_too_less == True):
                dist_to_chk = 2.0
