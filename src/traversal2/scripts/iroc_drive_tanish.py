...
class Drive:
    def __init__(self):
        ...
        rospy.Subscriber("/motion", WheelRpm,
                         self.autonomous_motion_callback, queue_size=10)

        # Publishers
        self.pwm_pub = rospy.Publisher(
            'motor_pwm', Int32MultiArray, queue_size=10)
        ...
        self.pwm_msg.layout.dim[0].size = self.pwm_msg.layout.dim[0].stride = len(
            self.pwm_msg.data)
        ...
        self.modeupbtn = 7
        self.prev_modeupbtn = 0
        self.modednbtn = 6
        self.prev_modednbtn = 0
        ...
        self.kp_steer = 1
        self.qsize = 1
        # to implement ledc type control
        self.vel_prev = queue.Queue(self.qsize)
        # to implement ledc type control
        self.omega_prev = queue.Queue(self.qsize)
        ...
        if (self.state == False):  # i.e. rover is in manual mode
            ...
            if (self.steer_islocked == True):
                ...
                if (msg.buttons[self.modeupbtn] == 1):
                    self.prev_modeupbtn = 1
                else:
                    if (self.prev_modeupbtn == 1):
                        if (self.mode < 4):
                            self.mode = self.mode + 1
                    self.prev_modeupbtn = 0

                if (msg.buttons[self.modednbtn] == 1):
                    self.prev_modednbtn = 1
                else:
                    if (self.prev_modednbtn == 1):
                        if (self.mode > 0):
                            self.mode = self.mode - 1
                    self.prev_modednbtn = 0
                # differential drive axes
                self.drive_ctrl = [
                    msg.axes[self.fb_axis], msg.axes[self.lr_axis]]
    ...
    def drive(self):
        ...
        if (self.steering_complete == True and self.steer_islocked == True):
            ...
            if (self.rotinplace == True):  # if wheels are in rotate in place orientation
                ...
                self.pwm_msg.data = [
                    int(vel), -int(vel), int(vel), -int(vel), 0, 0, 0, 0]
                ...
                # the math part is to only print upto 2 decimals
                print("Velocity:", (avg_velocity//0.001)/1000)
                print("Omega: ", (avg_omega//0.001)/1000)
                print("Mode: ", self.mode)
                print()

                self.pwm_msg.data = [-int(avg_velocity-avg_omega), int(avg_velocity+avg_omega), -int(
                    avg_velocity-avg_omega), int(avg_velocity+avg_omega), 0, 0, 0, 0]  # convention at the top
