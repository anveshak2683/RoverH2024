...
class Drive:
    def __init__(self):
        ...
        for i in range(len(l)):
            l[i] = int(l[i])
        self.init_dir = l   #[1, 1, 1, 1, 1, 1, 1, 1]       #to be changed only during an emergency or hurry during testing
        #make sure front is [+,+,+,+] and so on using the .ino file (for consistency across all codes)

        n = int(f.readline().rstrip())
        self.max_steer_pwm = n  #127
        f.close()
...
#        rospy.Subscriber('motion', WheelRpm, self.autonomous_callback)  
        ...
        self.d_arr = [35,70,110,140,180,225, 255]
        self.s_arr = [self.max_steer_pwm for i in range(len(self.d_arr))] #no modes in steering       
