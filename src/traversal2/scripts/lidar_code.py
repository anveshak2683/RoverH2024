...
#red colour
lower_range=np.array([105,138,123])
...
class Lidar_Detection():
    ...
    def lidar_callback(self, data):
        ...
        right_check_sum = 0
        print("check1")
        data.ranges = np.nan_to_num(data.ranges, copy = True, nan = 0.0, posinf = 0.0, neginf = 1000)
    ...
    def color_detection(self): 
        ...
        if x1 != 0 and y1 != 0 and x2 ==0 and x3 ==0 and y2 == 0 and y3 ==0:
            self.depth_color = self.depth_image[x1,y1]
            print("red detected")
        #elif x1 == 0 and y1 == 0 and x2 !=0 and x3 ==0 and y2!= 0 and y3 ==0:
         #   self.depth_color=self.depth_image[x2,y2]
        #elif x1 == 0 and y1 == 0 and x2 ==0 and x3 !=0 and y2== 0 and y3 !=0:
         #   self.depth_color=self.depth_image[x3,y3]
        else:
            self.color_detected= False
    ...
    def main(self):
        ...
        if self.counter == threshold and self.color_detected == False :
            self.prevpoint = self.midpoint_list[-1]
            x = []
            y = []
            # Iterate over each sublist in the list of lists
            for sublist in self.midpoint_list:
                x.append(sublist[0])
                y.append(sublist[1])
            ...
            self.midpoint_list = []
            self.midpoint_list.append(self.prevpoint)
            print(self.midpoint_list)
            distance = self.spline_distance(x, y)
            print("dist",distance)
            self.distance_spline = self.distance_spline + distance 
            self.counter = 0
            print(f"The distance along the cubic spline is: {self.distance:.2f}")
        if self.color_detected == True and self.counter_final == 0 :
            self.counter_final += 1
            self.distance = self.distance + self.depth_cardboard
