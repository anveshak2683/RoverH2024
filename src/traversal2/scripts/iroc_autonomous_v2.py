...
class habibo():
    def __init__(self):
        ...
        self.goals_x = [0.619, 2.24, 2.74, 8.34, 6.22]
        self.goals_y = [1.11, 1.81, 0.83, 2.92, 1.68]
                ...
                for box in boxes:
                    ...
                    try:
                        if (int(c) == 0 or int(c) == 1):
                            if (left+right)/2 < 575 or (left + right)/2 > 45:
                                depth_value = self.depth_image[int(
                                    (top+bottom)/2), int((left+right)/2)]
                                ...
                                elif abs(height) > 24 and depth_value < 1.2 and self.goal_counter >3:
...
# rospy.sleep(0.25)
    ...
    def go_to_goal(self, n):
        print("self.ik", self.ik)
        if self.goal_counter == 3 and self.ik==0:
        
            print("Reached 3")
            # now we ik_dynamic.py code will start publishing ik values to manipulator so manipulator will run
            ik_start.publish(True)
            # g = WheelRpm()
            # g.vel = 0
            # g.omega = 0
            
            if self.ik_over_ah:
                ik_start.publish(False)
                print("ik pickup stopped")
                self.ik+= 1
                #g = WheelRpm()
                #g.vel =-30
                #for i in range(50):
        ...
        elif self.goal_counter==4 and self.ik==1:
            # g= WheelRpm()
            # g.vel=0
            # g.omega=0
            # now ik_dynamic.py will start publishing ik values to manipulator
            ik_start_drop.publish(True)
            if self.ik_over_ah:
                ik_start_drop.publish(False)
                self.ik+=1
                print("ik drop stopped")
            
        
                
        else:
            # this stops the ik from running
            ik_start.publish(False)
            ...
            print("goal_vec: ", goal_vec)
            desired_turn = math.atan(
                (goal_vec[1]-self.odom_self[1])/(goal_vec[0]-self.odom_self[0]))
            desired_turn = (desired_turn)*180/math.pi
