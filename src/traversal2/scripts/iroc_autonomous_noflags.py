...
class habibo():
    def __init__(self):
        ...
        self.goals_x = [1.32, 2.48, 3.45, 8.15, 6.22]
        self.goals_y = [1.42, 1.95, 0.41, 3.33, 1.68]
                    ...
                    try:
                        if (int(c) == 0 or int(c) == 1):
                            if (left+right)/2 < 675 or (left + right)/2 > 45:
                                ...
                                elif abs(height) > 30 and depth_value < 1.2:
...
# rospy.sleep(0.25)
    ...
    def frame_transformation(self, x, y, z):
        ...
        br.sendTransform((x, y+0.5, z), (0, 0, 0, 1),
                         rospy.Time.now(), "obstacle", "zed2i_base_link")
        listener = tf.TransformListener()
        trans = []
        rospy.sleep(0.25)
        try:
            (trans, rot) = listener.lookupTransform(
                "map", "obstacle", rospy.Time(0))
            print("trans", trans)
