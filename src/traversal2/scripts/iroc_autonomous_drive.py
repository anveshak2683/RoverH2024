...
class habibo():
	def __init__(self):
		...
		self.model1 = YOLO('/home/nvidia/galileo2024/src/traversal2/scripts/best.pt')
		self.model2 = YOLO("/home/nvidia/kavin_stuffs/train/weights/best.pt")
		self.cv_image = []
		self.current_pitch = self.current_roll = self.current_yaw = 0.0
		self.initial_pitch = self.initial_roll = self.initial_yaw = 0.0
		self.goals_x = [0,0]
		self.goals_y = [0,0]
		self.goals_x[0] = float(input("Enter the x1 coordinate: "))
		self.goals_x[1] = float(input("Enter the x2 coordinate: "))
		self.goals_y[0] = float(input("Enter the y1 coordinate: "))
		self.goals_y[1] = float(input("Enter the y2 coordinate: "))
		...
		if self.cv_image is not None:
			...
			for r in results:
				...
				for box in boxes:
					...
					try:
						depth_value = self.depth_image[int((left+right)/2), int((top+bottom)/2)]
						print("depth_value cube ", depth_value)
						depth_value = depth_value[1]
	...
	def cube_detect_YOLO(self):
		print("cube_detcetion")
		#self.model2 = YOLO("/home/nvidia/galileo2024/src/traversal2/scripts/best.pt")
		#        runtime_parameters = sl.RuntimeParameters()
		#       left_image = sl.Mat()
		#      depth_map = sl.Mat()
		self.ret_cube = False
...
#			self.cv_image = np.transpose(self.cv_image)
#			transposed_image = np.transpose(self.cv_image, (1, 0, 2))
#			self.cv_image = transposed_image 
			img = self.cv_image
			print("Dimensions of self.cv_image", np.shape(self.cv_image))
			results = self.model2.predict(img, conf=0.25, max_det=10)
#			print("results of prediction:", results)
#			transposed_image = np.transpose(self.cv_image, (1, 0, 2))
#			self.cv_image = transposed_image
			for r in results:
				annotator = Annotator(np.ascontiguousarray(img))  # Ensure image is contiguous
				...
				for box in boxes:
					b = box.xyxy[0]  # get box coordinates in (left, top, right, bottom) format
					c = box.cls
					annotator.box_label(b, self.model2.names[int(c)])
					# self.ret_cube = True
					# Draw rectangle around the object
					left, top, right, bottom = map(int, b)
					cv2.rectangle(img, (left, top), (right, bottom), (0, 255, 0), 2)
					try:
						...
						print("depthimage shape", np.shape(self.depth_image))
						#self.depth_image = np.transpose(self.depth_image)
...
#						self.cv_image = transposed_image
#						cv2.rectangle(img, (left, top), (right, bottom), (0, 255, 0), 2)
						if abs(height)>30 and depth_value<2.2:
							self.ret_cube = True
							self.obstacle_avoidance(left, right)
							rospy.sleep(0.25)
	...
	def posn_transformation(self, p_x, p_y):
		...
		cord_x=(p_x-c_x)/f_x
		cord_y=(p_y-c_y)/f_y
		print("sum and len of depth_crater", sum(self.depth_crater), len(self.depth_crater))
		cord_z=sum(self.depth_crater)/len(self.depth_crater)
...
#		rospy.sleep(0.25)
	...
	def go_to_goal(self):
		...
		if self.obs_avd_completed == True:
			...
			rospy.sleep(2)
			self.obs_avd_completed = False
		if self.obs_avd_completed == False:
