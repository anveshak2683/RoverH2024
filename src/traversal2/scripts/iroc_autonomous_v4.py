                    ...
                    try:
                        if (int(c) == 0 or int(c) == 1):
                            if (left+right)/2 < 540 or (left + right)/2 > 100:
                                depth_value = self.depth_image[int(
                                    (top+bottom)/2), int((left+right)/2)]
                                #if not np.isnan(depth_value_prev):
                                  #  self.depth_value_tanish=depth_value_prev
                                   # depth_value=self.depth_value_tanish
                                #else:
                                  #  depth_value=self.depth_value_tanish
...
# rospy.sleep(0.25)
    ...
    def safe_obstacle_avoidance(self,l,r,d):
        ...
        if (l+r)/2 <370 and (l+r)/2 > 270 :
            g.vel = int(kp * d)
            g.omega = 0	
            if np.isnan(d) or np.isinf(abs(d)):
                print("nan came")
                g.vel = 0
        elif (l+r)/2 <270 :
            g.omega = int(min(20,kp_rot * (320-(l+r)/2)))
            g.vel = 0
        elif (l+r)/2 >370:
            g.omega = int(max(-20, kp_rot*(320-(l+r)/2)))
            g.vel = 0
        ...
        if self.goal_counter == 3 and self.ik_over_ah ==0:
            ...
            if self.obs_avd_completed == True:
                ...
                if abs(self.current_yaw - self.desired_angle_gtg) > 6:
                    ...
                    g.omega = 0
            if(self.ret_guiding_cube == False) or (self.ret_guiding_cube_override == True):
                vel_pub.publish(g)
