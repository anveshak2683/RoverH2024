...
class Drive:
    def __init__(self):
        ...
        self.gui_pub = rospy.Publisher('gui_data_drive', Float32MultiArray, queue_size=10)
...
#        rospy.Subscriber('motion', WheelRpm, self.autonomous_callback)  
        ...
        self.gui_manual_ackerman = 0
        self.gui_steer_same      = 0
        self.gui_steer_opp       = 0
        self.gui_individual_FL = 0
        self.gui_individual_FR = 0
        self.gui_individual_BL = 0
        self.gui_individual_BR = 0
...
#        else:
    ...
    def spin(self):
        ...
        while not rospy.is_shutdown():
            ...
            msg.data = [self.mode, self.steer_islocked_numeric, self.full_potential_islocked_numeric, self.steering_ctrl_locked[0], self.steering_ctrl_locked[1], self.steering_ctrl_locked[2], self.gui_manual_ackerman, self.steering_ctrl_unlocked[0], self.steering_ctrl_unlocked[1], self.gui_steer_same, self.gui_steer_opp, self.gui_individual_FL, self.gui_individual_FR, self.gui_individual_BL, self.gui_individual_BR]
            #print(msg)
            #msg = Float32MultiArray()
            #msg = np.asanyarray(msg)
    ...
    def steering(self):
        self.steer_islocked_numeric = int(self.steer_islocked)
        self.full_potential_islocked_numeric = int(self.full_potential_islocked)
    ...
    def drive(self):
        if(self.steering_complete == True and self.steer_islocked == True and self.full_potential_islocked == True):
            ...
            self.pwm_msg.layout = std_msgs.MultiArrayLayout()
            self.pwm_msg.layout.data_offset = 0
            self.pwm_msg.layout.dim = [ std_msgs.MultiArrayDimension() ]
            self.pwm_msg.layout.dim[0].size = self.pwm_msg.layout.dim[0].stride = len(self.pwm_msg.data)
            self.pwm_msg.layout.dim[0].label = 'write'
        else:
            pass
